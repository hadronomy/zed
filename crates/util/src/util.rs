...
/// The checkout that produced this binary, resolved at runtime by walking up
/// from the executable to the first ancestor that contains a `.git` entry (a
/// directory in a normal clone, a file in a git worktree or submodule). Cargo
/// and corgi both place built binaries under `<repo>/target/<profile>/`, so the
/// repository root is always an ancestor.
///
